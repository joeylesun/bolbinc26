...
def save_fsm_config(new_times):
    """ Helper to save FSM times to JSON """
    # Update the global dict
    FSM_TIMES.update(new_times)

    # Save to file
    import json
    path = get_config_path("fsm_config.json")
    try:
        with open(path, 'w') as f:
            json.dump(FSM_TIMES, f, indent=4)
        print(f"[Config] Saved FSM times to {path}")
    except Exception as e:
